...
ECPS_DATA = {
    "fl": [
        ("ftp", "ФТП", "Федеральные торговые площадки", "2400"),
        ("rosreestr_fl", "Росреестр (ФЛ)", "Для покупки квартиры", "1800"),
        ("epgu", "ЕПГУ", "Госуслуги и государственные порталы", "1800"),
        ("efrsfdyul", "ЕФРСФДЮЛ", "Федеральный ресурс юридических лиц", "1800"),
        ("fts", "ФТС", "Таможенная служба", "1800"),
        ("fts_alta", "ФТС Альта-Софт", "Таможня + программное обеспечение Альта-Софт", "3500"),
        ("egais", "ЕГАИС", "Учёт оборота алкогольной продукции", "2000"),
        ("rosreestr_ki", "Росреестр (кадастровый инженер)", "Подписание межевых и техпланов", "1800"),
        ("rosreestr_au", "Росреестр (арбитражный управляющий)", "Работа с делами о банкротстве", "1800"),
        ("rzd", "РЖД", "Электронная торговая площадка ОАО «РЖД»", "2800"),
        ("cdt", "ЦДТ", "Центр дистанционных торгов", "5900"),
        ("utender", "uTender", "Коммерческая ЭТП", "4000"),
        ("fabrikant", "Фабрикант", "Коммерческая ЭТП", "3500"),
        ("b2b", "B2B-Center", "Крупнейшая коммерческая площадка", "3500"),
        ("regtorg", "Регторг", "Коммерческая площадка", "3800"),
        ("uetp", "УЭТП", "Уральская ЭТП", "3800"),
        ("aist", "АИСТ", "Коммерческая ЭТП", "4000"),
        ("tender_ug", "Тендер ug", "Коммерческая площадка", "4200"),
        ("gpb", "ГПБ", "Закупки Газпромбанка", "4400"),
        ("alfalot", "Альфалот", "Коммерческая площадка", "3700"),
        ("atc", "Аукц. тендерный центр", "Коммерческая площадка", "3200"),
        ("center_real", "Центр реализации", "Продажа имущества (в т.ч. банкротство)", "2500"),
        ("etp_esp", "ЭТП ЭСП", "Поволжская площадка", "2500"),
        ("fis_frd", "ФИС ФРДО", "Для образовательных учреждений всех уровней", "2500"),
        ("crypto_embed", "Вшитая лицензия Крипто Про", "Дополнительно к ЭЦП", "+900"),
    ],
    "ip": [
        ("ip_note", "ИП", "Не выпускаем новые ЭЦП на ИП. Возможна только продление по действующей.", "1000"),
    ],
    "ul": [
        ("ul_note", "ООО / ЮЛ", "Не выпускаем новые ЭЦП на ООО. Возможна только продление по действующей.", "1000"),
    ]
}
...
# =============================
CRYPTO_ITEMS = [
    ("rt_lite", "Рутокен Lite", "Носитель ЭЦП начального уровня", "1900 ₽"),
    ("rt_3", "Рутокен 3.0", "Носитель ЭЦП с расширенными возможностями", "2600 ₽"),
    ...
    ("cp_arm", "Крипто АРМ (бессрочная)", "Лицензия для подписания документов ЭЦП", "2600 ₽"),
    ("pc_setup", "Настройка ПК под ЭЦП", "Установка драйверов, КриптоПро, тестирование. Настройка ПК по удаленному доступу, либо у нас в офисе", "2000 ₽"),
]
...
PROCUREMENTS = {
    "44fz": [
        ("base_44", "Базовое сопровождение по 44-ФЗ", "Полное оформление закупки: от подготовки заявки до подписания контракта. 1. Подготовка заявки 2. Подача заявки 3. Проведение аукциона 4. Подписание контракта", "от 6500 ₽"),
        ("pp2571", "Подтверждение опыта по ПП 2571", "Прохождение аккредитации на 1 ЭТП для подтверждения опыта", "2500 ₽"),
        ("struct_menu", "Структурированная форма закупки", "Анализ и заполнение показателей структуры", "см. подменю"),
        ("urgent_44", "Срочность (<1 дня)", "Оформление заявки менее чем за 1 рабочий день до окончания подачи", "+2000 ₽"),
        ("93_12", "Закупка по ч.12 ст.93", "Закупка единственного поставщика по 44-ФЗ. Размещение предложения на ЭТП", "от 4000 ₽"),
    ],
    "223fz": [
        ("base_223", "Базовое сопровождение по 223-ФЗ", "Полное сопровождение закупки. 1. Подготовка заявки 2. Подача заявки 3. Проведение аукциона 4. Подписание контракта", "от 9000 ₽"),
        ("urgent_223", "Срочность", "Ускоренное оформление", "+2000 ₽"),
    ],
    "com": [
        ("base_com", "Коммерческие торги", "Подготовка заявки на коммерческих ЭТП. 1. Подготовка заявки 2. Подача заявки 3. Проведение аукциона 4. Подписание контракта", "от 9000 ₽"),
        ("urgent_com", "Срочность", "Ускоренное оформление", "+2000 ₽"),
    ],
    "bankrot": [
        ...
        ("urgent_bankrot", "Срочность", "Ускоренное оформление", "+2000 ₽"),
    ],
    "bereza": [
...
STRUCTURE_ITEMS = [
    ("struct_1_40", "Показатели 1–40", "Анализ и заполнение до 40 показателей", "+2000 ₽"),
    ("struct_41_80", "Показатели 41–80", "Анализ и заполнение до 80 показателей", "+3000 ₽"),
    ("struct_81_120", "Показатели 81–120", "Анализ и заполнение до 120 показателей", "+4000 ₽"),
    ("struct_121_160", "Показатели 121–160", "Анализ и заполнение до 160 показателей", "+5000 ₽"),
]

OTHER_SERVICES = {
    "reg": [
        ("eruz", "Регистрация в ЕРУЗ", "Получение аккредитации в Едином реестре участников закупок", "4500 ₽"),
        ("com_plat", "Регистрация на коммерческой площадке", "Аккредитация на одной коммерческой ЭТП", "от 5000 ₽"),
        ("dop_one", "Доп. требования (1 площадка)", "Прохождение доп. аккредитации на одной площадки", "2500 ₽"),
        ("dop_all", "Доп. требования (все площадки)", "Прохождение доп. аккредитации на всех площадках (8 Федеральный торговых площадок)", "6500 ₽"),
    ],
    "complex": [
        ("complex_1", "Комплексное сопровождение (1 мес.)", "Полное сопровождение всех закупок клиента в течение месяца. 1. Максимум 20 Закопок за месяц 2. Делаем МЧД для участия в торгах 3. Проходим регистрацию на коммерческих площадках по присланным закупкам", "20 000 ₽ + 1%"),
        ("complex_6", "Комплексное сопровождение (6 мес.)", "Ежемесячная абонентская поддержка. 1. Максимум 20 Закопок в месяц 2. Делаем МЧД для участия в торгах 3. Выпускае ЭЦП 4. Проходим регистрацию на коммерческих площадках по присланным закупкам 5. Рассылка тендеров", "17 000 ₽/мес + 1%"),
    ],
    "other": [
        ("act", "Электронное актирование", "Подписание актов по заключённым контрактам", "2000 ₽"),
        ("mchd", "МЧД", "Выдача машиночитаемой доверенности. В основном для торгов, но можем сделать любую", "1500 ₽"),
        ("trade_long", "Участие в торгах (>5 часов)", "Сопровождение длительных торгов", "1500 ₽/час (раб.), 5000 ₽/час (нераб.)"),
        ("ast_gos", "АСТ-ГОЗ (гособоронзаказ)", "Получение доступа и настройка ПК для гособоронзаказа", "12 000 ₽"),
        ("etprf_gpb", "ETPRF.RU / Специализированная на ГПБ", "Получение доступа к закрытым площадкам", "12 000 ₽"),
        ("fas", "Жалоба в ФАС", "Подготовка и подача жалобы на действия заказчика", "от 10 000 ₽"),
